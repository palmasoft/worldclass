...
class ContratoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $contrato->one_representante = variable::find(13)->valor;
        $contrato->one_representante_nacionalidad = variable::find(14)->valor;
        $contrato->one_representante_ciudad = variable::find(21)->valor;
        $contrato->one_representante_cedula = variable::find(15)->valor;
        $contrato->one_empresa = variable::find(16)->valor;
        $contrato->one_empresa_comercial = variable::find(17)->valor;
        $contrato->one_empresa_ruc = variable::find(18)->valor;
        $contrato->one_empresa_direccion = variable::find(19)->valor;
        $contrato->one_empresa_legal = variable::find(20)->valor;
        ...
        $contrato->noches = $request->noches;
        ...
        $enunciados = Enunciado::where('activo','>',0)->get();

        foreach ($enunciados as $enunciado) {
            $encon = new ContratoEnunciado();

            $encon->contrato_id = $contrato->id;
            $encon->enunciado = $enunciado->enunciado;
          ...
          || (floatval($c->servicioInternacional) > 0)))
        {
          $estadoFinal = "SEPARACION";
            ...
            $estadoFinal = "CASH";
                ...
                //Fórmula modificada el 16/07/2019.
                $descuento = $c->pagoInicial-($c->gastoLegal*floatval($vg[2]->valor)+$c->club*floatval($vg[5]->valor)+$c->xpack*floatval($vg[1]->valor)+$c->servicioInternacional*floatval($vg[4]->valor)+$c->boordingCard*floatval($vg[3]->valor));

                @$operacion = ($descuento * 100) /($c->anosPactados*floatval($vg[0]->valor));
            
            /*
            @$operacion = (($c->pagoInicial - ($c->gastoLegal*floatval($vg[2]->valor)+$c->xpack*floatval($vg[1]->valor)+$c->servicioInternacional*floatval($vg[4]->valor)+
