...
@section('content')
    <div class="container m-2">
        ...
        <div class="row">
            <div class="col-12">
                <a href="{{ url('/excelcarteradiaria') }}" target="_blank" class="btn btn-dark">
                    Informe Diario
                </a>
                <a href="{{ url('/excelcarteramensual') }}" target="_blank" class="btn btn-secondary">
                    Informe Mensual
                </a>
            </div>
        </div>
            ...
            @if (@$contratos)
                @foreach ($contratos as $item)
                    <div class="col-12 mt-1 mb-1">
                        <contratos-component base="{{ url('/crud') }}" contratoid="{{ $item->id }}" destino="{{ url('/') }}" cartera="1"></contratos-component>
                    </div>
                @endforeach
            @endif
    ...
    <script>
        function buscarXcs(){
            ...
            window.location.href = "<?php echo url('/cartera'); ?>" + "/" + a1 + "|" + a2 + "|" + a3 + "|" + a4 + "|" + a5 + "|" + a6;
        }
    </script>
@endsection