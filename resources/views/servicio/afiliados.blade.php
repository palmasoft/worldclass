...
@section('content')
            ...
            @if (@$contratos)
                @foreach ($contratos as $item)
                    <div class="col-12 mt-1 mb-1">
                        <afiliado-component base="{{ url('/crud') }}" contratoid="{{ $item->id }}" titularid="{{ $item->titular }}" destino="{{ url('/') }}"></afiliado-component>
                    </div>
                @endforeach
            @endif
    ...
    <script>
        function buscarXcs(){
            ...
            window.location.href = "<?php echo url('/afiliados'); ?>" + "/" + a1 + "|" + a2 + "|" + a3 + "|" + a4 + "|" + a5 + "|" + a6;
        }
    </script>
@endsection